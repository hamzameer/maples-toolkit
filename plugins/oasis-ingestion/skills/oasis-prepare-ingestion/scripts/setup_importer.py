...
def main():
    ...
    try:
        if sys.version_info < (3, 10):  # noqa: UP036 - the distributed skill supports Python 3.10+
            raise ValueError("Python 3.10 or newer is required")
        fingerprint = verify_bundle()
        if args.check:
            check_environment(dest)
            print(
                "PASS: bundled runtime, installed SDK, and dependency environment verified; no server calls"
            )
            return
        if dest.exists() or dest.is_symlink():
            raise ValueError(
                "Environment destination already exists; use --check or select a fresh directory"
            )
        ...
        if installer == "uv":
            ...
            if not uv:
                raise ValueError(
                    "uv is not installed; select pip with a Python that supports venv/ensurepip"
                )
            ...
            try:
                ...
                raise ValueError(
                    "This Python lacks ensurepip. Use an approved Python with venv support, or install/use uv. No system changes were made."
                ) from exc
        ...
        subprocess.run(
            ...
            + ["install"]
            + target
            + ["--require-hashes", "--only-binary=:all:", "-r", str(RUNTIME / "requirements.lock")],
            check=True,
        )
        subprocess.run(
            pip
            + ["install"]
            + target
            + ["--no-deps", str(RUNTIME / "elephant_sdk-3.1.1-py3-none-any.whl")],
            check=True,
        )
        ...
        with zipfile.ZipFile(RUNTIME / "elephant_sdk-3.1.1-py3-none-any.whl") as wheel:
            expected = {
                n[len("elephant/") :]: hashlib.sha256(wheel.read(n)).hexdigest()
                for n in wheel.namelist()
                if n.startswith("elephant/") and n.endswith((".py", ".typed"))
            }
        if snapshot["sdk_files"] != expected:
            raise ValueError("Installed SDK differs from the bundled wheel")
        (dest / "oasis-importer-setup.json").write_text(
            json.dumps({"bundle": fingerprint, "environment": snapshot}, indent=2) + "\n"
        )
        print(
            "PASS: importer environment created; no server calls. Use --check to verify it later."
        )
