...
TYPES = {
    'transcript_srt': {'.srt'}, 'transcript_vtt': {'.vtt'},
    'transcript_txt': {'.txt'}, 'notes_txt': {'.txt'},
    'notes_md': {'.md'}, 'notes_json': {'.json'},
...
def validate(manifest, root):
    ...
    for line, row in enumerate(reader, 2):
        ...
        if (rel.is_absolute() or '..' in rel.parts or '\\' in str(rel)
                or ':' in str(rel) or not rel.parts):
        ...
        if size == 0:
            ...
            continue
        files.append({'path': str(rel), 'sha256': digest.hexdigest(), 'bytes': size,
                      'file_type': kind})
    ...
    return {'status': 'FAIL' if errors else 'PASS', 'errors': errors,
            ...
            'counts': {'cohorts': len(cohorts), 'learners': len(learners),
                       'encounters': len(encounters), 'files': len(files),
                       'by_type': dict(counts)}, 'files': files,
            'limits': ['No semantic mapping or document readability verification',
