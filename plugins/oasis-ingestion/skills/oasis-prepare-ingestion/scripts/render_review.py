...
from validate_manifest import validate, FIELDS


def selected_inventory(manifest_path, data_dir):
    report = validate(manifest_path, data_dir)
    if report['status'] != 'PASS':
        raise ValueError('selected manifest failed validation; inspect its protected validation report')
    ...
    for row in csv.DictReader(io.StringIO(raw.decode('utf-8'), newline='')):
        ...
        kind = row['file_type']
        modality = ('notes' if kind.startswith('notes_') else
                    'transcripts' if kind.startswith('transcript_') else
                    kind if kind in ('video', 'audio') else 'other')
        entry['counts'][modality] += 1
        entry['counts']['files'] += 1
    rows = []
    for key, entry in sorted(groups.items()):
        rows.append(list(key) + [len(entry['encounters'])] +
                    [entry['counts'][k] for k in ('notes', 'transcripts', 'video', 'audio', 'other', 'files')])
...
def render(data, selected=None):
    ...
    if manifest is not None and not all(integer(manifest[k]) for k in ('rows', 'encounters', 'learners', 'derivatives')):
    ...
    if data['phase'] == 'final':
        ...
        for report_key, count_key in (('rows', 'files'), ('encounters', 'encounters'), ('learners', 'learners')):
    ...
    def table(headers, rows):
        return '<div class="table"><table><thead><tr>' + ''.join(
            '<th scope="col">' + esc(h) + '</th>' for h in headers
        ) + '</tr></thead><tbody>' + ''.join(
            '<tr>' + ''.join('<td>' + esc(c) + '</td>' for c in row) + '</tr>'
            for row in rows
        ) + '</tbody></table></div>'

    body = '<h1>' + esc(data['title']) + '</h1><p class="status">' + esc(
        data['phase'].capitalize() + ' inventory · ' + data['review_status']
    ) + '</p><p>' + esc(data['summary']) + '</p>'
    ...
    if selected is not None:
        ...
        body += table(['Cohort', 'Activity', 'Case', 'Encounters', 'Notes', 'Transcripts', 'Videos', 'Audio', 'Other', 'Total files'], selected['rows'] + [['All selected', '', ''] + totals])
        body += '<details><summary>Selected manifest SHA-256</summary><pre>' + esc(selected['sha256']) + '</pre></details>'
    ...
    body += ('<ul>' + ''.join('<li>' + esc(q) + '</li>' for q in data['questions']) + '</ul>'
             if data['questions'] else '<p>No open questions recorded.</p>')
    body += '<h2>Verification</h2>' + table(
        ['Check', 'Status', 'Evidence / limits'],
        [[r[k] for k in ('check', 'status', 'evidence')] for r in data['checks']])
    body += '<details><summary>Detailed references and hashes</summary><pre>' + esc(
        data.get('details', 'No additional details recorded.')
...
def main():
    ...
    parser.add_argument('--manifest', type=Path, help='Final selected ingest.csv; required for final review')
