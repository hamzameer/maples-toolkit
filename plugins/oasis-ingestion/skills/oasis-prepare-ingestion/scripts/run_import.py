...
from runtime_common import RUNTIME, check_environment, python_in, verify_bundle
...
def read_config(path, windows_acl_verified=False):
    ...
    if os.name == "posix":
        for candidate in (path, path.parent):
            ...
            if st.st_uid != os.getuid() or stat.S_IMODE(st.st_mode) & 0o077:
                raise ValueError(
                    "Connection file and its directory must be owned by you and private (0600/0700)"
                )
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not sep or name not in ("ELEPHANT_API_KEY", "ELEPHANT_API_URL") or name in values:
            raise ValueError(
                "Config needs exactly one entry for each required variable; no other assignments"
            )
        if value.startswith(('"', "'")):
    ...
    try:
        ...
        if args.action != "dry-run":
            if not args.config or not args.expected_url:
                raise ValueError(
                    "Connection requires --config and the exact reviewed --expected-url"
                )
        ...
        if args.action != "check-connection":
            ...
            if report["status"] != "PASS":
                raise ValueError(
                    "Manifest validation failed; run the validator for protected details"
                )
        if args.action == "ingest":
            if not args.confirm_ingest or not args.review_validation:
                raise ValueError(
                    "Ingestion requires explicit confirmation and the reviewed validation receipt"
                )
            reviewed = json.loads(args.review_validation.read_text())
            if reviewed != report:
                raise ValueError(
                    "Manifest or input files differ from the reviewed validation receipt"
                )
        os.umask(0o077)
        ...
        with (args.receipt_dir / "import.log").open("x", encoding="utf-8") as stream:
            ...
            with contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
                try:
                    ...
                    if args.action == "check-connection":
                        if values is None:
                            raise ValueError("Connection values were not loaded")
                        from elephant import ElephantClient  # ty: ignore[unresolved-import]

                        client = ElephantClient(
                            api_key=values["ELEPHANT_API_KEY"], base_url=values["ELEPHANT_API_URL"]
                        )
                        client.get_health()
                        client.get_file_types()
                        print(
                            "Health and file-types reads succeeded. Write role and namespace isolation are not established."
                        )
                    else:
                    ...
                    status = 1
                    print(
                        "Operation failed; raw exception withheld to avoid credential disclosure."
                    )
                finally:
                    log.flush()
        ...
        receipt = {
            ...
            "status": "COMMAND_SUCCEEDED_RECONCILIATION_REQUIRED"
            if args.action == "ingest" and status == 0
            else "PASS"
            if status == 0
            else "FAIL",
        ...
        parser.exit(
            1,
            "Preflight failed: verify bundle/environment, private config, exact target, reviewed inputs, confirmation, and fresh receipt path. No import started.\n",
        )
